...
from .Addmodules.C2fDualConv import DualConv
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        if isinstance(m, (Detect, Segment, Pose, Detect_AFPN4, Detect_AFPN3, Detect_ASFF, Detect_FRM, Detect_dyhead,
                          CLLAHead,  Detect_DySnakeConv, Segment_DySnakeConv,Pose_DySnakeConv, RepHead,
                          Segment_DBB, Detect_DBB, Pose_DBB, OBB, Detect_FASFF, RFAHead, RFAPose, RFASegment, Detect_Adown,RepPose,Pose_AFPN4)):
            s = 640  # 2x min stride
            m.inplace = self.inplace
            forward = lambda x: self.forward(x)[0] if isinstance(m, (Segment, Segment_DySnakeConv, Pose_DySnakeConv, Pose, Pose_DBB, Segment_DBB,
                                                                     RFASegment, RFAPose, OBB,RepPose,Pose_AFPN4)) else self.forward(x)
            try:
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck,
                 ...
                 RepC3, C2f_ACmix, C2f_AKConv, AKConv,C2f_DCNv2, DCNv2, DiverseBranchBlock, C2f_DBB,
                 C2f_DLKA, C2f_DSConv, C2f_DWRSeg, C2f_EMA, C2f_FLA, C2f_iAFF, C2f_iRMB, C2f_MLCA, C2f_ODConv, ODConv2d, RCSOSA, CSPStage, RepConv, RFAConv, C2f_RFAConv,
                 SAConv2d, C2f_SAConv, C2f_SCConv, C2f_SENetV1, VoVGSCSP, SPDConv, C2f_TripletAt, GSConv,
                 C2f_iRMB_EMA, C2f_CGA, SimConv, nn.Conv2d, C2f_MSBlock, C2f_OREPA, C2f_DCNv4, CSPPC, CSPHet, RepNCSPELAN4_Dual,
                 C2f_DCNv3, Blocks, ConvNormLayer, C2f_FasterBlock, RepNCSPELAN4_high, SPPELAN, C2f_DCNv3_DLKA,
                 RepNCSPELAN4_low,C2f_DynamicConv,DynamicConv,C2f_GhostModule_DynamicConv,
                 RepNCSPELAN4_DynamicConv,RepNCSPELAN4_iAFF,RepNCSPELAN4_OREPA,RepNCSPELAN4_UniRepLKNetBlock,PKIBlock,RepNCSPELAN4_CAA,RepNCSPELAN4_ContextGuidedBlock,DualConv,
                 RepNCSPELAN4_SENetV2,RepNCSPELAN4CIB
                 ):
            ...
            if m in (BottleneckCSP, C1, C2, C2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3, C2f_ACmix, C2f_AKConv,
                     ...
                     VoVGSCSP, C2f_TripletAt, C2f_iRMB_EMA, C2f_CGA, C2f_MSBlock, C2f_OREPA, C2f_DCNv4,
                     CSPPC, CSPHet, C2f_DCNv3, Blocks, C2f_FasterBlock, C2f_DCNv3_DLKA):
        ...
        #---------------------------------------注意力机制------------------------
                   ...
                   deformable_LKA, FocalModulation, FocusedLinearAttention, iRMB, LSKA, TripletAttention, OREPA,PSA,
        ...
        # ------------------------------ASF-YOLO--------------------------------
            ...
            c2 = sum(ch[x] for x in f)
                   ...
                   Detect_FRM, Detect_dyhead, CLLAHead,Detect_DySnakeConv, Segment_DySnakeConv,Pose_DySnakeConv,
                   Pose_DBB, Segment_DBB, Detect_DBB, Detect_FASFF, RFAHead, RFASegment, RFAPose, RepHead,RepPose,Pose_AFPN4,
                   Detect_Adown):
...
def guess_model_task(model):
    ...
    def cfg2task(cfg):
        ...
        if m in ('segment', 'segment_dysnakeconv', 'segment_dbb', 'rfasegment'):
            return "segment"
        if m in ('pose', 'pose_dbb', 'rfapose','pose_dysnakeconv','reppose','pose_afpn4'):
            return "pose"
        if m == "obb":
            return "obb"
        else:
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Segment, Segment_DySnakeConv, Segment_DBB, RFASegment)):
                ...
                return "classify"
            elif isinstance(m, (Pose, Pose_DBB, RFAPose,Pose_DySnakeConv,RepPose,Pose_AFPN4)):
                return "pose"
            elif isinstance(m, OBB):
                return "obb"
            elif isinstance(m, (Detect, WorldDetect, Detect_AFPN4, Detect_AFPN3, Detect_ASFF, Detect_FRM,
                                Detect_dyhead, CLLAHead,Detect_DySnakeConv, Detect_DBB, Detect_FASFF,
