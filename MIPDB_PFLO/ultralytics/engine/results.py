...
class Results(SimpleClass):
    ...
    def plot(
        ...
        # Plot Pose results
        ob_nums = len(self.keypoints.data)

        random_colors = [(r, g, b) if (r, g, b) != (0, 0, 255) else (
            random.randint(1, 255), random.randint(1, 255), random.randint(1, 255))
                         for _ in range(ob_nums)
                         for r, g, b in [(random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))]]
        if self.keypoints is not None:
            cls_list = reversed(self.boxes.cls)
            if cls_list.size(0) > 0:  # 检查 cls_list 是否包含任何值
                i = 0
                for k in reversed(self.keypoints.data):
                    ...
                    if cls == 1:
                        ...
                        annotator.kpts(k, self.orig_shape, radius=kpt_radius, kpt_line=kpt_line,
                                       color_line=random_colors[i])
                    else:
                        color = [0, 0, 255]
                        annotator.kpts(k, self.orig_shape, radius=kpt_radius, kpt_line=kpt_line, color_line=color)
                    i += 1;
