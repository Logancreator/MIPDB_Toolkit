...
__all__ = ['Detect_AFPN4','Pose_AFPN4']
...
class AFPN(nn.Module):
    def __init__(self, in_channels=[256, 512, 768, 1024], out_channels=512):
        super(AFPN, self).__init__()

        self.fp16_enabled = False

        self.conv0 = BasicConv(in_channels[0], in_channels[0] // 4, 1)
        self.conv1 = BasicConv(in_channels[1], in_channels[1] // 4, 1)
        self.conv2 = BasicConv(in_channels[2], in_channels[2] // 4, 1)
        self.conv3 = BasicConv(in_channels[3], in_channels[3] // 4, 1)

        self.body = nn.Sequential(
            BlockBody([in_channels[0] // 4, in_channels[1] // 4, in_channels[2] // 4, in_channels[3] // 4])
        )
        ...
        self.conv33 = BasicConv(in_channels[3] // 4, out_channels, 1)
...
class Detect_AFPN4(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(512, c2, 3), Conv(c2, c2, 3), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch
        )
        self.cv3 = nn.ModuleList(nn.Sequential(Conv(512, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc, 1)) for x in ch)
        self.dfl = DFL(self.reg_max) if self.reg_max > 1 else nn.Identity()
        self.AFPN = AFPN(ch)  # 初始化 AFPN 时，传入输入通道数
...
class Pose_AFPN4(Detect_AFPN4):
    ...
    def __init__(self, nc=80, kpt_shape=(17, 3), ch=()):
        ...
        self.detect = Detect_AFPN4.forward
...
if __name__ == "__main__":
    if __name__ == "__main__":
        # Sample input
        x = [torch.randn(1, 256, 80, 80), torch.randn(1, 512, 40, 40), torch.randn(1, 768, 20, 20),
             torch.randn(1, 768, 10, 10)]
        model = Detect_AFPN4(nc=80, ch=[256,512,768,768])
        output = model(x)
