...
from ultralytics.nn.modules.block import C3, C2f, RepBottleneck

__all__ = ['RepNCSPELAN4_UniRepLKNetBlock']
...
class RepCSP_UniRepLKNetBlock(nn.Module):
    """Rep CSP Bottleneck with 3 convolutions."""

    def __init__(self, c1, c2, n=1, k=7, shortcut=True, g=1, e=0.5):
        """Initializes RepNCSPELAN4_UniRepLKNetBlock layer with given channels, repetitions, shortcut, groups and expansion ratio."""
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        ...
        self.m = nn.ModuleList(UniRepLKNetBlock(c_, k) for _ in range(n))

    def forward(self, x):
        """Forward pass through RepNCSPELAN4_UniRepLKNetBlock layer."""
        x1 = self.cv1(x)
        for m in self.m:
            x1 = m(x1)
...
class RepNCSPELAN4_UniRepLKNetBlock(nn.Module):
    """CSP-ELAN."""
    ...
    def __init__(self, c1, c2, c3, c4, n=1):
        ...
        self.cv2 = nn.Sequential(RepCSP_UniRepLKNetBlock(c3 // 2, c4, n), Conv(c4, c4, 3, 1))
        self.cv3 = nn.Sequential(RepCSP_UniRepLKNetBlock(c4, c4, n), Conv(c4, c4, 3, 1))
